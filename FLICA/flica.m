...
% ABOVE LINE NEEDS TESTING!
    ...
    if isequal(opts.lambda_dims{k},'R')
        ...
        if any(lambda{k}==inf)     % for missing data begin
           lam=lambda{k};
           lam(find(lam==inf))=0;
           lambda{k}=lam;
           prior_lambda_b{k}(find(lam==0))=1e-18;
           prior_lambda_c{k}(find(lam==0))=1e12;
        end                       % for missing data end 
...
for its = 1:opts.maxits
    ...
    MK =zeros(L,R,K);   % FOR EACH MODALITY
    ...
    for k=1:K
        ...
        MK(:,:,k)=DD(k) * diag(W{k}) * double(X{k}' * Y{k}) * diag(lambda_R{k});  % ADD BY HJ
    end
...
%     end
    for r=R:-1:1
        ...
        for k=1:K
        HK(:,r,k) = H_colcov(:,:,tmp_R_to_NH(r)) * MK(:,r,k);  % ADD BY HJ
        end 
    end
