...
for k=1:K
    rescaleHK(k,:)=polarity./rms(M.HK(order,:,k),2)';
end
...
for f = fieldnames(M)' % Loop over fields in M
    ...
    switch(fn)
        ...
        case 'tmpMK' % reorder dim2         % ADD BY HJ
            M.tmpMK = M.tmpMK(order,:,:);
            
            
        case 'HK' % reorder dim2           % ADD BY HJ
            for k=1:K
                M.HKscale(:,:,k) = diag(rescaleHK(k,:))*M.HK(order,:,k);
            end
            M.HK = M.HK(order,:,:);
